...
// Declare template
let indexTmpl = Template.TS_customer,
    contactTmpl = Template.TS_customerContact,
    formTmpl = Template.TS_customerForm,
    showTmpl = Template.TS_customerShow;
...
formTmpl.onCreated(function () {
    this.autorun(()=> {
        // Lookup value
        this.subscribe('ts.lookupValueByNames', ['Gender', 'Contact Type']);

        let currentData = Template.currentData();
        if (currentData) {
            this.subscribe('ts.customerById', currentData.customerId);
        }
    });
});
...
AutoForm.addHooks(['TS_customerForm'], hooksObject);
